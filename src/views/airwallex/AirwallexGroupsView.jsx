...
import { faPlus, faPen, faTrash, faTimes, faLayerGroup, faAlignLeft, faSpinner, faCreditCard, faUser } from "@fortawesome/free-solid-svg-icons";
	...
	return (
		<div className={`bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-700/50 transition-all duration-200 cursor-pointer ${expanded ? "ring-1 ring-blue-100" : "hover:shadow-md"}`} onClick={() => setExpanded(!expanded)}>
			...
			<div className="space-y-2">
				<div>
					<span className="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1.5 block">{t("airwallexAccounts")}</span>
	...
	return (
		<tr className="hover:bg-gray-50/50 transition-colors cursor-pointer" onClick={() => setExpanded(!expanded)}>
			...
			<td className="py-2 px-6 text-xs text-gray-500 dark:text-gray-400 max-w-xs" title={item.remark}>
				<div className={`line-clamp-3 ${expanded ? "line-clamp-none" : ""}`}>{item.remark || "-"}</div>
			</td>
			...
			<div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col animate-in fade-in zoom-in duration-300 mx-2">
				...
				<div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
					<div className="grid grid-cols-1 gap-y-6">
						...
						<InputRow icon={faCreditCard} label={t("airwallexAccounts")} noBorder>
							<Select
								isMulti
								value={form.stripe_list}
								onChange={(val) => setForm((v) => ({ ...v, stripe_list: val }))}
								options={currentStripeOptions}
								placeholder={t("selectAirwallexAccounts")}
								className="w-full mt-1"
								styles={{
									control: (base, state) => ({
										...base,
...
export default function AirwallexGroupsView() {
	const { t } = useI18n();
	const dispatch = useDispatch();
		...
		<div className="p-4 md:p-6 max-w-[1600px] mx-auto">
			<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
				<div>
					...
					<p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t("airwallexAccountGroupingDesc")}</p>
				</div>
				<button
					onClick={() => {
					...
					<table className="w-full">
						<thead>
							<tr className="bg-gray-50/50 dark:bg-gray-700/50 border-b border-gray-100 dark:border-gray-700/50 text-left">
								...
								<th className="py-2 px-6 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{t("airwallexAccounts")}</th>
