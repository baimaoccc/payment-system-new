...
import klarnaIcon from "../../assets/klarna-icon.jpg";
import idealIcon from "../../assets/ideal-icon.png";
import bancontactIcon from "../../assets/bancontact-icon.png";
...
export function AirwallexAccountModal({ isOpen, onClose, onSuccess, initialData = null, readOnly = false }) {
	...
	const handleSubmit = async (e) => {
		...
		const payload = {
			...formData,
			paymentChannel: formData.paymentChannel && formData.paymentChannel.length > 0 ? formData.paymentChannel.map((ch) => `[${ch}]`).join(",") : "",
					...
					<div className="mb-10">
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
							...
							<InputRow icon={faCreditCard} label={t("paymentType")} className="md:col-span-1" noBorder>
								<Select value={formData.paymentType} onChange={(val) => setFormData((prev) => ({ ...prev, paymentType: val }))} options={paymentTypeOptions.map((option) => ({ value: option.value, label: option.label }))} placeholder={t("selectPaymentType") || "Select Payment Type"} isDisabled={readOnly} className="w-full" />
							</InputRow>

							<InputRow icon={faCreditCard} label={t("paymentChannel") || "Payment Channel"} className="md:col-span-1" noBorder>
								<Select
									isMulti
									value={formData.paymentChannel || []}
									onChange={(opts) => {
										console.log("Selected payment channels:", opts);
										setFormData((prev) => ({ ...prev, paymentChannel: opts || [] }));
									}}
									options={[
										{
											value: "klarna",
							...
							<InputRow icon={faShieldAlt} label={t("st_type") || "Type"} noBorder>
								<Select value={formData.type} onChange={(val) => setFormData((prev) => ({ ...prev, type: val }))} options={[{ value: 2, label: t("airwallexTypeNormal") || "airwallex" }, { value: 0, label: t("stTypePhishing") || "Phishing" }]} isDisabled={readOnly} className="w-full" />
							</InputRow>
						</div>
					</div>
					...
					<div className="mb-8">
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
							<InputRow icon={faClock} label={t("subscriptionFrequency") || "扣款频率"} noBorder={true}>
								<Select value={formData.frequency} onChange={(val) => setFormData((prev) => ({ ...prev, frequency: val }))} options={[{ value: "day", label: t("frequency_day") || "每天" }, { value: "week", label: t("frequency_week") || "每周" }, { value: "month", label: t("frequency_month") || "每月" }]} placeholder={t("pleaseSelect") || "请选择"} isDisabled={readOnly} className="w-full" isClearable={true} />
							</InputRow>
