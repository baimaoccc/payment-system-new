		...
		<div className="p-6">
			<div className="bg-white rounded-2xl shadow p-4">
				<div className="flex items-center justify-between mb-3">
					<h3 className="text-sm font-semibold text-gray-900">{t("airwallexLogs") || "Airwallex Logs"}</h3>
				</div>
									...
									list.map((item) => {
										...
										return (
											<tr key={item.id} className="hover:bg-gray-50 transition-colors border-t">
												<td className="p-3 overflow-hidden text-gray-600 font-mono">{item.id}</td>
												<td className="p-3 overflow-hidden truncate">{item.stripe_id || "-"}</td>
												<td className="p-3 overflow-hidden truncate">
													<span className="bg-gray-100 px-1.5 py-0.5 rounded text-[10px] font-medium">{item.type || "INFO"}</span>
												</td>
												<td className="p-3 overflow-hidden truncate font-mono text-gray-500">{typeof item.content === "object" ? JSON.stringify(item.content) : item.content}</td>
												<td className="p-3 overflow-hidden truncate text-gray-500">{createDate}</td>
												<td className="p-3">
				...
				<div className="fixed inset-0 z-50 flex items-center justify-center">
					...
					<div className="relative overflow-hidden bg-white rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col z-10 m-4 animate-in fade-in zoom-in duration-200">
						...
						<div className="p-4 border-t flex justify-end">
							<button onClick={closeLogModal} className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
								{t("close")}
							</button>
						</div>
					</div>
