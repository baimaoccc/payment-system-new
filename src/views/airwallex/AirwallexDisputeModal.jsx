						...
						<div className="p-6">
										...
										<tbody className="divide-y divide-gray-100">
											{list.map((item, idx) => (
												<tr key={item.id || idx} className="hover:bg-gray-50 transition-colors">
													<td className="px-4 py-3 font-medium text-gray-900">{formatCurrency(item.amount, item.currency)}</td>
													...
													<td className="px-4 py-3 text-gray-600 capitalize">{item.reason ? t(`dispute_reason_${item.reason}`) || item.reason.replace(/_/g, " ") : "-"}</td>
													<td className="px-4 py-3 font-mono text-xs text-gray-500">{typeof item.charge === "string" ? item.charge : item.charge?.id || "-"}</td>
													<td className="px-4 py-3 text-gray-500">{item.evidence_details?.due_by ? new Date(item.evidence_details.due_by * 1000).toLocaleDateString() : "-"}</td>
													<td className="px-4 py-3 text-gray-500">{item.created ? new Date(item.created * 1000).toLocaleString() : "-"}</td>
												</tr>
											))}
										</tbody>
									</table>
							...
							{hasMore && (
								<div className="mt-4 text-center">
									<button onClick={() => loadData(true)} disabled={loadingMore} className="px-4 py-2 bg-white border border-gray-200 text-gray-600 rounded-lg text-sm hover:bg-gray-50 transition-colors disabled:opacity-50">
										{loadingMore ? (
											<>
