...
export function AirwallexDisputeModal({ isOpen, onClose, accountId }) {
	const { t } = useI18n();
						...
						<div className="p-6">
										...
										<tbody className="divide-y divide-gray-100 dark:divide-gray-700/50">
											{list.map((item, idx) => (
												<tr key={item.id || idx} className="hover:bg-gray-50 dark:bg-gray-900 transition-colors">
													<td className="px-4 py-3 font-medium text-gray-900 dark:text-gray-100">{formatCurrency(item.amount, item.currency)}</td>
													<td className="px-4 py-3">
														<span
															className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize ${
																item.status === "won" ? "bg-green-50 text-green-700" : item.status === "lost" ? "bg-red-50 text-red-700" : "bg-yellow-50 text-yellow-700"
															}`}>
															{t(`dispute_status_${item.status}`) || item.status}
														</span>
													</td>
													<td className="px-4 py-3 text-gray-600 dark:text-gray-400 capitalize">{item.reason ? t(`dispute_reason_${item.reason}`) || item.reason.replace(/_/g, " ") : "-"}</td>
													<td className="px-4 py-3 font-mono text-xs text-gray-500 dark:text-gray-400">{typeof item.charge === "string" ? item.charge : item.charge?.id || "-"}</td>
													<td className="px-4 py-3 text-gray-500 dark:text-gray-400">{item.evidence_details?.due_by ? new Date(item.evidence_details.due_by * 1000).toLocaleDateString() : "-"}</td>
													<td className="px-4 py-3 text-gray-500 dark:text-gray-400">{item.created ? new Date(item.created * 1000).toLocaleString() : "-"}</td>
												</tr>
											))}
										</tbody>
									</table>
							...
							{hasMore && (
								<div className="mt-4 text-center">
									<button onClick={() => loadData(true)} disabled={loadingMore} className="px-4 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/50 text-gray-600 dark:text-gray-400 rounded-lg text-sm hover:bg-gray-50 dark:bg-gray-900 transition-colors disabled:opacity-50">
										{loadingMore ? (
											<>
