...
import { faLayerGroup, faPlus, faTrash, faEye, faSearch, faSpinner, faTimes, faCheck, faGlobeAmericas, faEdit } from "@fortawesome/free-solid-svg-icons";
...
const InputRow = ({ icon, label, children, className = "" }) => (
	<div className={`flex items-start gap-4 ${className}`}>
		...
		<div className="flex-1 border-b border-gray-200 dark:border-gray-700/50 focus-within:border-blue-600 transition-colors pb-2 pt-1 relative">
...
const MobileWhitelistGroupCard = ({ group, onEdit, onDelete, t, lang }) => {
	const [expanded, setExpanded] = useState(false);
	const hasManyItems = group.fz_json && Array.isArray(group.fz_json) && group.fz_json.length > 12;

	return (
		<div className={`bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border transition-all duration-200 cursor-pointer ${expanded ? "border-blue-500 ring-1 ring-blue-500" : "border-gray-100 dark:border-gray-700/50 hover:shadow-md"}`} onClick={() => setExpanded(!expanded)}>
			<div className="flex justify-between items-start mb-3">
				<div className="flex-1 min-w-0 mr-3">
					<div className="flex items-center gap-2 mb-1">
						<span className="bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 px-1.5 py-0.5 rounded text-xs font-mono shrink-0">#{group.id}</span>
						<h3 className="font-bold text-gray-900 dark:text-gray-100 line-clamp-1 break-all">{group.name}</h3>
					</div>
				</div>
...
export default function AirwallexWhitelistGroupsView() {
	...
	const [createData, setCreateData] = useState({ name: "", selectedCountries: "" });
	...
	useEffect(() => {
		if (allCountries.length > 0) {
			const mapped = allCountries.map((c) => ({
				code: c.alpha2 || String(c.id),
				name: lang === "zh" ? c.name_cn || c.name : c.name_en || c.name,
				flag: c.flag,
			}));
	...
	return (
		<div className="p-6">
			<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700/50">
				<div>
					<h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">{t("whitelistGroupsTitle")}</h1>
					<p className="text-gray-500 dark:text-gray-400 text-sm mt-0.5">{t("airwallexWhitelistGroupsSub")}</p>
				</div>
			...
			{loading ? (
				<div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700/50 overflow-hidden mt-4 p-12 text-center text-gray-500 dark:text-gray-400 flex flex-col items-center gap-3">
				...
				<div className="mt-4 space-y-3">
					{groups.map((group) => (
						<MobileWhitelistGroupCard key={group.id} group={group} onEdit={handleEdit} onDelete={handleDelete} t={t} lang={lang} />
					))}
					<div className="pt-2">
				...
				<div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700/50 overflow-hidden mt-4">
					<div className="overflow-x-auto">
						<table className="w-full text-sm text-left">
							...
							<tbody className="divide-y divide-gray-100 dark:divide-gray-700/50">
								{groups.map((group) => (
									<tr key={group.id} className="hover:bg-gray-50 dark:bg-gray-900 transition-colors text-[12px]">
										<td className="px-6 py-2 font-mono text-gray-500 dark:text-gray-400">#{group.id}</td>
										...
										<td className="px-6 py-2 text-gray-500 dark:text-gray-400">{group.createtime ? new Date(group.createtime * 1000).toLocaleString() : "-"}</td>
			...
			{viewModalOpen && (
				<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
					<div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
						<div className="px-6 py-4 border-b border-gray-100 dark:border-gray-700/50 flex justify-between items-center bg-gray-50 dark:bg-gray-900">
							<h3 className="font-bold text-gray-900 dark:text-gray-100">{t("details")}</h3>
							<button onClick={() => setViewModalOpen(false)} className="text-gray-400 hover:text-gray-600 dark:text-gray-400">
						...
						<div className="p-6 max-h-[60vh] overflow-y-auto">
							{viewLoading ? (
								...
								<div className="space-y-4">
									<div>
										<label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider block mb-1">{t("name")}</label>
										<div className="text-gray-900 dark:text-gray-100 font-medium">{viewData.name}</div>
									</div>
									<div>
										<label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider block mb-1">{t("st_whitelist")}</label>
									...
									{viewData.createtime && (
										<div>
											<label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider block mb-1">{t("crt")}</label>
											<div className="text-gray-600 dark:text-gray-400 text-sm">{new Date(viewData.createtime * 1000).toLocaleString()}</div>
										</div>
									)}
								</div>
							) : (
								<div className="text-center text-gray-500 dark:text-gray-400">{t("noData")}</div>
							)}
						</div>
						<div className="px-6 py-4 bg-gray-50 dark:bg-gray-900 border-t border-gray-100 dark:border-gray-700/50 flex justify-end">
							<button onClick={() => setViewModalOpen(false)} className="px-4 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/50 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-50 dark:bg-gray-900 font-medium text-sm">
								{t("close")}
							</button>
						</div>
					</div>
			...
			{createModalOpen && (
				<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
					<div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
						<div className="px-8 py-6 border-b border-gray-100 dark:border-gray-700/50 bg-white dark:bg-gray-800">
							<div className="flex justify-between items-start">
								...
								<button onClick={() => setCreateModalOpen(false)} className="text-gray-400 hover:text-gray-600 dark:text-gray-400 transition-colors p-2 rounded-full hover:bg-gray-50 dark:bg-gray-900">
						...
						<div className="flex-1 overflow-y-auto custom-scrollbar">
							<div className="p-6">
								<InputRow icon={faLayerGroup} label={t("name")}>
									<input type="text" value={createData.name} onChange={(e) => setCreateData((prev) => ({ ...prev, name: e.target.value }))} className="w-full outline-none bg-transparent text-gray-900 dark:text-gray-100 placeholder-gray-300 py-1" placeholder={t("enterGroupName")} />
								</InputRow>
						...
						<div className="p-6 border-t border-gray-100 dark:border-gray-700/50 bg-gray-50 dark:bg-gray-900 flex justify-end gap-3">
							<button onClick={() => setCreateModalOpen(false)} className="px-6 py-2.5 text-sm font-medium text-gray-600 dark:text-gray-400 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/50 rounded-lg hover:bg-gray-50 dark:bg-gray-900 hover:text-gray-900 dark:text-gray-100 transition-all shadow-sm">
								{t("cancel")}
							</button>
