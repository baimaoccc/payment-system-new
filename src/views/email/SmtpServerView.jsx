		...
		<div className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-700/50">
			<div className="flex justify-between items-start mb-3">
				<div className="flex items-center gap-3">
					...
					<div>
						<div className="font-bold text-gray-900 dark:text-gray-100">{item.host}</div>
						<div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{item.username}</div>
					</div>
				</div>
				<div className={`px-2 py-1 rounded text-xs font-medium ${item.secure ? "bg-green-50 text-green-600" : "bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400"}`}>{item.secure || "None"}</div>
			</div>
			...
			{expanded && (
				<div className="border-t border-gray-50 dark:border-gray-700/50 pt-3 mb-4 space-y-3 animate-fade-in">
					<div>
						<div className="text-xs text-gray-400 mb-1">{t("smtpFromName")}</div>
						<div className="text-sm text-gray-700 dark:text-gray-300">{item.from_name || "-"}</div>
					</div>
					<div className="grid grid-cols-2 gap-4">
						<div>
							<div className="text-xs text-gray-400 mb-1">{t("crt")}</div>
							<div className="text-xs text-gray-500 dark:text-gray-400">{formatDate(item.createtime)}</div>
						</div>
						<div>
							<div className="text-xs text-gray-400 mb-1">{t("upd")}</div>
							<div className="text-xs text-gray-500 dark:text-gray-400">{formatDate(item.updatetime)}</div>
						</div>
					</div>
				</div>
			)}
			...
			<div className="flex items-center justify-between pt-3 border-t border-gray-100 dark:border-gray-700/50">
				<button onClick={() => setExpanded(!expanded)} className="text-xs text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:text-gray-300 flex items-center gap-1">
					{expanded ? (
						<>
...
function SmtpFormModal({ open, initial, onClose, onSave, t, readOnly }) {
	...
	return (
		<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
			<div className="bg-white dark:bg-gray-800 overflow-hidden rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col">
				<div className="flex items-center justify-between p-6 border-b border-gray-100 dark:border-gray-700/50">
					<h3 className="text-xl font-bold text-gray-800 dark:text-gray-100">{readOnly ? t("viewSmtpServer") : initial ? t("editSmtpServer") : t("addSmtpServer")}</h3>
				...
				<form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6">
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
						<InputRow label={t("smtpHost")}>
							<input type="text" className="w-full px-4 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700/50 rounded-lg focus:bg-white dark:bg-gray-800 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 text-sm" value={form.host} onChange={(e) => setForm({ ...form, host: e.target.value })} disabled={readOnly} />
						</InputRow>
						<InputRow label={t("smtpPort")}>
							<input type="text" className="w-full px-4 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700/50 rounded-lg focus:bg-white dark:bg-gray-800 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 text-sm" value={form.port} onChange={(e) => setForm({ ...form, port: e.target.value })} disabled={readOnly} />
						</InputRow>
					</div>

					<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
						<InputRow label={t("smtpUsername")}>
							<input type="text" className="w-full px-4 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700/50 rounded-lg focus:bg-white dark:bg-gray-800 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 text-sm" value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })} disabled={readOnly} />
						</InputRow>
						<InputRow label={t("smtpPassword")}>
							<div className="flex items-center relative">
								<input type={showPassword ? "text" : "password"} className="w-full px-4 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700/50 rounded-lg focus:bg-white dark:bg-gray-800 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 text-sm" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} disabled={readOnly} />
								{!readOnly && (
									<button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute bottom-[-4px] right-2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:text-gray-400 focus:outline-none">
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
						<InputRow label={t("smtpFromName")}>
							<input type="text" className="w-full px-4 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700/50 rounded-lg focus:bg-white dark:bg-gray-800 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 text-sm" value={form.from_name} onChange={(e) => setForm({ ...form, from_name: e.target.value })} disabled={readOnly} />
						</InputRow>
						<InputRow label={t("smtpFromEmail")}>
							<input type="email" className="w-full px-4 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700/50 rounded-lg focus:bg-white dark:bg-gray-800 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 text-sm" value={form.from_email} onChange={(e) => setForm({ ...form, from_email: e.target.value })} disabled={readOnly} />
						</InputRow>
					</div>

					<InputRow label={t("smtpSecure")}>
						<input type="text" className="w-full px-4 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700/50 rounded-lg focus:bg-white dark:bg-gray-800 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 text-sm" value={form.secure || ""} disabled />
					</InputRow>
				</form>
		...
		<div className="p-4 md:p-6 max-w-[1600px] mx-auto">
			<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
				<div className="flex items-center gap-4">
					...
					<div>
						<h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{t("smtpServerManagement")}</h1>
						<p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t("smtpServerManagementDesc")}</p>
					</div>
				</div>
				<button
					onClick={() => {
						...
						<table className="min-w-full text-xs">
							<thead className="bg-gray-100 dark:bg-gray-700">
								<tr>
									...
									<th className="px-3 py-3 text-left font-medium text-gray-700 dark:text-gray-300 w-1/5">{t("smtpHost")}</th>
									<th className="px-3 py-3 text-left font-medium text-gray-700 dark:text-gray-300">{t("smtpUsername")}</th>
									<th className="px-3 py-3 text-left font-medium text-gray-700 dark:text-gray-300">{t("smtpFromEmail")}</th>
									<th className="px-3 py-3 text-left font-medium text-gray-700 dark:text-gray-300 w-1/6">{t("timeline")}</th>
									<th className="px-3 py-3 text-left font-medium text-gray-700 dark:text-gray-300 w-20">{t("actions")}</th>
								</tr>
							</thead>
							<tbody>
								{list.length > 0 ? (
									list.map((item) => (
										<tr key={item.id} className="border-t hover:bg-gray-50 dark:bg-gray-900">
											...
											<td className="p-3 align-top">
												<div className="font-bold text-gray-900 dark:text-gray-100 text-sm">{item.host}</div>
												<div className="text-gray-500 dark:text-gray-400 text-[11px]">{item.port}</div>
											</td>
											<td className="p-3 align-top">
												<div className="text-gray-800 dark:text-gray-100 text-xs truncate max-w-xs" title={item.username}>
													{item.username}
												</div>
											</td>
											<td className="p-3 align-top">
												<div className="text-gray-800 dark:text-gray-100 text-xs">{item.from_email}</div>
												<div className="text-gray-500 dark:text-gray-400 text-[11px]">{item.from_name}</div>
											</td>
											<td className="p-3 align-top">
