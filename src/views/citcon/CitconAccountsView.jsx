...
import { CitconAccountModal } from "./CitconAccountModal.jsx";
import { CitconWarningModal } from "./CitconWarningModal.jsx";
import { CitconDisputeModal } from "./CitconDisputeModal.jsx";
...
const FILTERS_CACHE_KEY = "citcon_accounts_filters_cache";
...
export function CitconAccountsView() {
	const { t } = useI18n();
	...
	const handleRemarkSave = async (item) => {
		...
		const safeFields = ["id", "status", "group_id", "comment", "token", "c_site_url", "max_money", "max_order", "description", "maximum_purchase_amount", "white_list", "country_group", "level", "type", "user_id", "paymentType"];
		const payload = {};
		...
		<div className="flex flex-col gap-1.5">
			{item.token && (
				<div className="group flex items-center gap-1 text-[10px] text-gray-500 dark:text-gray-400">
					<span className="font-medium w-6 flex-shrink-0">TK:</span>
					<span className="font-mono bg-gray-50 dark:bg-gray-900 px-1 rounded truncate flex-1" title={item.token}>
						{item.token.substring(0, 8)}...{item.token.slice(-4)}
					</span>
					<button
						onClick={(e) => {
							e.stopPropagation();
							handleCopy(item.token);
						}}
		...
		<div className="p-6">
			<div className="bg-white dark:bg-gray-800 rounded-2xl shadow p-4">
				<div className="relative flex flex-col gap-4 mb-4" ref={containerRef}>
					<h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100 flex-shrink-0">{t("citconAccounts") || "Citcon Accounts"}</h3>
											...
											<div className="p-3 flex items-center justify-between gap-3">
												<div className="flex-1 min-w-0">
													<div className="flex items-center gap-2 mb-1">
														...
														<span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${item.type === 4 ? "bg-purple-50 text-purple-600" : "bg-blue-50 text-blue-600"}`} title={t("st_type")}>
															{item.type === 4 ? t("citconTypeNormal") || "Citcon账号" : (item.type === 0 ? t("stTypePhishing") : t("stTypeNormal"))}
														</span>
													</div>
													...
													<td className="p-3 overflow-hidden">
														<div className="flex flex-col gap-1.5">
															<div className="flex items-center gap-2 flex-wrap">
																...
																<span className={`px-1.5 py-0.5 rounded text-[10px] font-medium bg-purple-50 text-purple-600`} title={t("st_type") || "Type"}>
																	{item.type === 4 ? t("citconTypeNormal") || "Citcon账号" : (item.type === 0 ? t("stTypePhishing") : t("stTypeNormal"))}
																</span>
															</div>
			...
			<CitconAccountModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSuccess={loadData} initialData={editingItem} readOnly={isReadOnly} />
			<CitconWarningModal isOpen={warningModalOpen} onClose={() => setWarningModalOpen(false)} accountId={selectedAccountId} />
			<CitconDisputeModal isOpen={disputeModalOpen} onClose={() => setDisputeModalOpen(false)} accountId={selectedAccountId} />
		</div>
	);
}
