			...
			<div className="flex justify-between items-start mb-3">
				<div className="flex-1 min-w-0 mr-3">
					<div className="flex items-center gap-2 mb-1">
						...
						<h3 className="font-bold text-gray-900 dark:text-gray-100 line-clamp-1 break-all">{item.category_name}</h3>
					</div>
				</div>
...
const DesktopCategoryRow = ({ item, onEdit, onDelete, t }) => {
	return (
		<tr className="hover:bg-gray-50/50 transition-colors">
			...
			<td className="py-2 px-6 text-xs font-medium text-gray-900 dark:text-gray-100">{item.category_name}</td>
...
function CategoryFormModal({ open, initial, onClose, onSave, t }) {
	...
	return (
		<div className="fixed inset-0 z-50 flex items-center justify-center">
			...
			<div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-hidden flex flex-col animate-in fade-in zoom-in duration-300 mx-2">
				<div className="flex items-center justify-between px-8 py-6 border-b border-gray-100 dark:border-gray-700/50 bg-white dark:bg-gray-800 z-10">
					<h3 className="text-xs font-bold text-gray-900 dark:text-gray-100">{initial ? t("editCategory") : t("addCategory")}</h3>
				...
				<div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
					<div className="space-y-6">
						<InputRow icon={faAlignLeft} label={t("categoryName")}>
							<input type="text" value={form.category_name} onChange={(e) => setForm((v) => ({ ...v, category_name: e.target.value }))} placeholder={t("enterName")} className="w-full outline-none bg-transparent text-gray-900 dark:text-gray-100 placeholder-gray-300 py-1" autoFocus />
						</InputRow>
					</div>
				</div>
	...
	return (
		<div className="p-4 md:p-6 max-w-[1600px] mx-auto">
			<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
				<div>
					<h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{t("productCategory")}</h1>
				</div>
				<button
					onClick={() => {
						setEditingItem(null);
					...
					<table className="w-full">
						<thead>
							<tr className="bg-gray-50/50 dark:bg-gray-700/50 border-b border-gray-100 dark:border-gray-700/50 text-left">
								...
								<th className="py-2 px-6 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{t("categoryName")}</th>
