...
const MobileWebsiteCard = ({ item, onEdit, onDelete, t, isBatchMode, isSelected, onToggle, isAdmin }) => {
	...
	return (
		<div className={`bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-700/50 transition-all duration-200 cursor-pointer ${expanded ? "ring-1 ring-blue-100" : "hover:shadow-md"} ${isSelected ? "ring-2 ring-blue-500 bg-blue-50/30" : ""}`} onClick={() => (isBatchMode ? onToggle(item.id) : setExpanded(!expanded))}>
			<div className="flex justify-between items-start mb-3">
				{isBatchMode && (
					<div className="mr-3 pt-1">
						<input type="checkbox" checked={isSelected} readOnly className="w-5 h-5 rounded border-gray-300 dark:border-gray-700/50 text-blue-600 focus:ring-blue-500 pointer-events-none" />
					</div>
				)}
				<div className="flex-1 min-w-0 mr-3">
					<div className="flex items-center gap-2 mb-1">
						...
						<h3 className="font-bold text-gray-900 dark:text-gray-100 line-clamp-1 break-all">{item.domain_name}</h3>
					</div>
					<div className="flex items-center gap-2 mb-1">
						<div className="text-xs text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full inline-block">{item.website_system}</div>
						{item.username && <div className="text-xs text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded-full inline-block">{item.username}</div>}
					</div>
					<div className="text-xs text-gray-500 dark:text-gray-400 break-all">{item.url}</div>
				</div>
				{canAction && (
		...
		<tr className={`hover:bg-gray-50/50 transition-colors ${isSelected ? "bg-blue-50/30" : ""}`} onClick={() => isBatchMode && onToggle(item.id)}>
			{isBatchMode && (
				<td className="py-2 px-6 w-10" onClick={(e) => e.stopPropagation()}>
					<input type="checkbox" checked={isSelected} onChange={() => onToggle(item.id)} className="w-4 h-4 rounded border-gray-300 dark:border-gray-700/50 text-blue-600 focus:ring-blue-500" />
				</td>
			)}
			<td className="py-2 px-6 text-xs font-medium text-gray-900 dark:text-gray-100">{item.domain_name}</td>
			...
			<td className="py-2 px-6 text-xs text-gray-600 dark:text-gray-400">
				<span className="bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded-full">{item.website_system || "-"}</span>
			</td>
			<td className="py-2 px-6 text-xs text-gray-600 dark:text-gray-400">{item.username || "-"}</td>
			<td className="py-2 px-6 text-xs">
				<span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium ${item.status == 1 ? "bg-green-50 text-green-700" : item.status == 2 ? "bg-orange-50 text-orange-700" : "bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400"}`}>
...
function WebsiteFormModal({ open, initial, onClose, onSave, t }) {
	...
	return (
		<div className="fixed inset-0 z-50 flex items-center justify-center">
			...
			<div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col animate-in fade-in zoom-in duration-300 mx-2">
				<div className="p-6 border-b border-gray-100 dark:border-gray-700/50 flex justify-between items-center bg-gray-50/50">
					<h3 className="text-lg font-bold text-gray-900 dark:text-gray-100">{initial ? t("editWebsite") : t("addWebsite")}</h3>
				...
				<div className="p-6 space-y-6">
					...
					<InputRow icon={faGlobe} label={t("domainName")}>
						<input type="text" value={form.domain_name} onChange={(e) => setForm((v) => ({ ...v, domain_name: e.target.value }))} placeholder={t("domainPlaceholder")} className="w-full outline-none bg-transparent text-gray-900 dark:text-gray-100 placeholder-gray-300 py-1" autoFocus />
					</InputRow>
					<InputRow icon={faLink} label={t("websiteUrl")}>
						<input type="text" value={form.url} onChange={(e) => setForm((v) => ({ ...v, url: e.target.value }))} placeholder={t("urlPlaceholder")} className="w-full outline-none bg-transparent text-gray-900 dark:text-gray-100 placeholder-gray-300 py-1" />
					</InputRow>
					...
					<InputRow icon={faToggleOn} label={t("status")} noBorder={true}>
						{isAdvertiser ? (
							<div className="flex items-center h-full px-3 text-sm text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-900 rounded-lg border border-gray-100 dark:border-gray-700/50">{form.status == "1" ? t("active") : form.status == "0" ? t("inactive") : t("pendingReview") || "Pending Review"}</div>
						) : (
							<Select
								value={form.status}
	...
	return (
		<div className="p-4 md:p-8 mx-auto">
			...
			<div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
				<div>
					<h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{t("websiteManagement")}</h1>
				</div>
				<div className="flex items-center gap-3 flex-wrap md:flex-nowrap">
					{isBatchMode ? (
						<div className="flex items-center gap-2 animate-in fade-in slide-in-from-right-5 duration-200">
							<label className="flex items-center gap-2 mr-2 cursor-pointer md:hidden bg-gray-100 dark:bg-gray-700 px-3 py-1 rounded-lg">
								<input type="checkbox" checked={list.length > 0 && selectedIds.length === list.length} onChange={handleSelectAll} className="w-4 h-4 rounded border-gray-300 dark:border-gray-700/50 text-blue-600 focus:ring-blue-500" />
								<span className="text-sm font-medium text-gray-600 dark:text-gray-400">{t("all") || "All"}</span>
							</label>
							<span className="text-sm text-gray-500 dark:text-gray-400 mr-2 font-medium bg-gray-100 dark:bg-gray-700 px-3 py-1 rounded-lg">
							...
							<button onClick={toggleBatchMode} disabled={batchProcessing} className="px-4 py-2.5 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 rounded-xl font-medium hover:bg-gray-200 transition-all text-xs">
								{t("cancelBatch")}
							</button>
						</div>
					) : (
						<>
							...
							{isSuperAdminOrAdmin && (
								<button onClick={toggleBatchMode} className="flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700/50 rounded-xl font-medium hover:bg-gray-50 dark:bg-gray-900 hover:border-gray-300 dark:border-gray-700/50 transition-all text-xs shadow-sm">
									<FontAwesomeIcon icon={faCheckCircle} className="text-blue-500" />
									<span>{t("batchAudit")}</span>
								</button>
							)}

							<button onClick={handleDownloadTemplate} disabled={isExporting} className="flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700/50 rounded-xl font-medium hover:bg-gray-50 dark:bg-gray-900 hover:border-gray-300 dark:border-gray-700/50 transition-all text-xs shadow-sm">
							...
							<div className="relative">
								...
								<button onClick={() => uploadInputRef.current?.click()} disabled={isUploading} className="flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700/50 rounded-xl font-medium hover:bg-gray-50 dark:bg-gray-900 hover:border-gray-300 dark:border-gray-700/50 transition-all text-xs shadow-sm">
			...
			{/* Table Card */}
			<div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl shadow-gray-200/50 border border-gray-100 dark:border-gray-700/50 overflow-hidden">
				{loading ? (
					...
					<>
						...
						<div className="hidden md:block overflow-x-auto">
							<table className="w-full">
								<thead>
									<tr className="bg-gray-50/50 dark:bg-gray-700/50 border-b border-gray-100 dark:border-gray-700/50">
										{isBatchMode && (
											<th className="py-2 px-8 w-10 flex items-center justify-center">
												<input type="checkbox" checked={list.length > 0 && selectedIds.length === list.length} onChange={handleSelectAll} className="w-4 h-4 rounded border-gray-300 dark:border-gray-700/50 text-blue-600 focus:ring-blue-500" />
											</th>
										)}
										<th className="py-2 px-6 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{t("domainName")}</th>
										<th className="py-2 px-6 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{t("websiteUrl")}</th>
										<th className="py-2 px-6 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{t("websiteSystem")}</th>
										<th className="py-2 px-6 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{t("username")}</th>
										<th className="py-2 px-6 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{t("status")}</th>
										<th className="py-2 px-6 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{t("upd")}</th>
										<th className="py-2 px-6 text-right text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{t("actions")}</th>
									</tr>
								</thead>
						...
						<div className="flex justify-between items-center px-6 py-4 border-t border-gray-100 dark:border-gray-700/50 bg-gray-50/50">
							{/* <div className="text-sm text-gray-500 dark:text-gray-400">
								{t("showing")} <span className="font-medium text-gray-900 dark:text-gray-100">{list.length}</span> {t("of")}{" "}
								<span className="font-medium text-gray-900 dark:text-gray-100">{pagination.total}</span> {t("results")}
							</div> */}
							<Pagination
